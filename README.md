...
- **Sub-Workflow 3: Consensus-Based/Convertfasta-based Identification**: Consensus sequences are generated using Flye and wf-amplicon in both de novo and variant calling modes. Additionally, FASTQ reads from the clinical isolates are converted to FASTA for direct ITSx processing. These four tools are used to perform species identification based on ITSx + BLAST searches.
